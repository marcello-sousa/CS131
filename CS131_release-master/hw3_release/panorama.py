...
def harris_corners(img, window_size=3, k=0.04):
    ...
    Ix2 = convolve(np.multiply(dx, dx), window)
    Iy2 = convolve(np.multiply(dy, dy), window)
...
def simple_descriptor(patch):
    ...
    ### YOUR CODE HERE
    if np.std(patch) != 0:
        X = (patch - np.mean(patch)) / np.std(patch)
    else:
        X = patch - np.mean(patch)
    feature = X.flatten()
    ### END YOUR CODE
...
def match_descriptors(desc1, desc2, threshold=0.5):
    ...
    ### YOUR CODE HERE
    dist_argsort = np.argsort(dists)
    
    for i in range(N):
        x, y = dists[i], dist_argsort[i]
        if x[y[0]] < threshold * x[y[1]]:
            matches.append((i, y[0]))
    ### END YOUR CODE
...
def ransac(keypoints1, keypoints2, matches, n_iters=200, threshold=20):
    ...
    for i in range(n_iters):
        ...
        H[:,2] = np.array([0, 0, 1])
        for j in range(N):
            if np.linalg.norm(matched2[j].dot(H)-matched1[j], ord=2)**2 < threshold:
                inliers += 1
                inx_inliers.append(j)
                k += 1
        if inliers > n_inliers:
            inx_inliers = np.asarray(inx_inliers)
            p1 = matched1[inx_inliers]
            p2 = matched2[inx_inliers]
            H_max = np.linalg.lstsq(p2, p1, rcond=None)[0]
            H_max[:,2] = np.array([0, 0, 1])
            n_inliers = inliers
            max_inliers = inx_inliers
