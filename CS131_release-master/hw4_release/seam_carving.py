...
def energy_function(image):
    ...
    out = np.abs(np.gradient(gray, axis=0)) + np.abs(np.gradient(gray, axis=1))
    ### END YOUR CODE
...
def compute_cost(image, energy, axis=1):
    ...
    ### YOUR CODE HERE
    for i in range(1, H):
        t = np.pad(cost[i-1], (1,1), 'maximum')
        m = t.strides[0]
        matrix = np.lib.stride_tricks.as_strided(t, shape=(3, W), strides=(m, m)).T
        paths[i] = np.argmin(matrix, axis=1) - 1
        cost[i] = energy[i] + np.min(matrix, axis=1)
...
def backtrack_seam(paths, end):
    ...
    for h in range(H-2, -1, -1):
        if paths[h-1, w] == -1: w -= 1
        elif paths[h-1, w] == 1: w += 1
        else: w = w
        seam[h-2] = w
    ### END YOUR CODE
...
def remove_seam(image, seam):
    ...
    out = image[x, y, z].reshape(H, W-1, C)
    ### END YOUR CODE
...
def duplicate_seam(image, seam):
    ...
    ### YOUR CODE HERE
    mask = np.ones((H, W + 1, C), dtype=bool)
    mask[np.arange(H), seam, :], mask[np.arange(H), seam + 1, :] = False, False
    i, j = np.where(np.tile(np.arange(W), H).reshape(H, W) != seam.reshape(H, 1))
    x, y = np.repeat(i, C), np.repeat(j, C)
    z =  np.tile(np.arange(C), len(x)//C) 
    out[np.where(mask == True)] = image[x, y, z]
    out[np.arange(H), seam, :] = image[np.arange(H), seam, :]
    out[np.arange(H), seam+1, :] = image[np.arange(H), seam, :]
    ### END YOUR CODE
...
def enlarge(image, size, axis=1, efunc=energy_function, cfunc=compute_cost):
    ...
    ### YOUR CODE HERE
    seam = find_seams(out, k = size - W, efunc = efunc, cfunc = cfunc)
#    seam = np.array([[1, 2, 0], [1, 2, 0], [1, 2, 0]])
    copy_seam = np.copy(seam)
    copy_seam = np.expand_dims(copy_seam, axis=2)
    
    for i in range(size - W):
        out = duplicate_seam(out, np.where(copy_seam == i+1)[1])
        copy_seam = duplicate_seam(copy_seam, np.where(copy_seam == i+1)[1])
    ### END YOUR CODE
...
def compute_forward_cost(image, energy):
    ...
    ### YOUR CODE HERE
    for i in range(1, H):
        previous_cost = np.pad(cost[i-1], (1,1), 'maximum')
        previous = np.pad(image[i-1], (1,1), 'constant', constant_values=(0, 0))
        current = np.pad(image[i], (1,1), 'constant', constant_values=(0, 0))
        m = previous_cost.strides[0]
        
        previous_cost = np.lib.stride_tricks.as_strided(previous_cost, shape=(3, W), strides=(m, m)).T
        current = np.lib.stride_tricks.as_strided(current, shape=(3, W), strides=(m, m)).T    
        previous = np.lib.stride_tricks.as_strided(previous, shape=(3, W), strides=(m, m)).T
        
        cu = np.abs(current[:,2] - current[:,0])
        cu[0], cu[-1] = 0, 0                          #edges
        cl = cu + np.abs(previous[:,1] - current[:,0])     
        cr = cu + np.abs(previous[:,1] - current[:,2])
        
        c = np.stack([cl, cu, cr], axis=1)
        
        new = np.copy(previous_cost)
        new += c
        
        paths[i] = np.argmin(new, axis=1) - 1
        cost[i] = energy[i] + np.min(new, axis=1)
    ### END YOUR CODE
...
def remove_object(image, mask):
    ...
    out[x, y] = np.max(out)
  #  out = out[np.where(out != -1)].reshape(image.shape[0], image.shape[1] - len(np.unique(y)), image.shape[2])
    size = len(np.unique(y))
    #print(image.shape)
    out = reduce(out, out.shape[1] - size)
    #print(out.shape)
    out = enlarge(out, out.shape[1] + size)
    ### END YOUR CODE
