...
class PCA(object):
    ...
    def _eigen_decomp(self, X):
        ...
        e_vals, e_vecs = np.linalg.eigh(np.cov(X.T))
...
class LDA(object):
    ...
    def _within_class_scatter(self, X, y):
        ...
        for i in np.unique(y):
            ...
            ind = np.where(y == i)[0]
            mean = np.mean(X[ind,:], axis = 0)
            X_centered = X[ind,:] - mean
            S_i = np.cov(X_centered.T)
            scatter_within += S_i
            # END YOUR CODE
    ...
    def _between_class_scatter(self, X, y):
        ...
        for i in np.unique(y):
            # YOUR CODE HERE
            ind = np.where(y == i)[0]
            mu_i = np.mean(X[ind, :], axis=0)
            S_b = len(ind)*np.multiply((mu_i - mu).reshape(D, 1), (mu_i - mu).reshape(1, D))
            scatter_between += S_b
            # END YOUR CODE
