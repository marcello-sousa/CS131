...
def compute_distances(X1, X2):
    ...
    x1 = np.sum(X1**2, axis=1, keepdims=True)
    x2 = np.sum(X2**2, axis=1)
    x1x2 = np.dot(X1, X2.T)
    dists = np.sqrt(x1 - 2*x1x2 + x2)
    # END YOUR CODE
...
def predict_labels(dists, y_train, k=1):
    ...
    for i in range(num_test):
        ...
        # YOUR CODE HERE
        closest_y.append(y_train[np.argsort(dists[i])[:k]])
        ocurrences = np.bincount(np.ravel(closest_y))
        y_pred[i] = np.argmax(ocurrences)
        # END YOUR CODE
...
def split_folds(X_train, y_train, num_folds):
    ...
    X_split = np.split(X_train, num_folds)
    y_split = np.split(y_train, num_folds)
    
    for i in range(num_folds):
        X_trains[i:,:] = np.concatenate(X_split[:i] + X_split[i+1:])
        y_trains[i:,:] = np.concatenate(y_split[:i] + y_split[i+1:])
        X_vals[i:,:] = X_split[i]
        y_vals[i:,:] = y_split[i]
